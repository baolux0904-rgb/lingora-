      ...
      <div className="flex items-center border rounded-md p-1 gap-1 mb-6 w-fit" style={{ backgroundColor: "var(--color-primary-soft)", borderColor: "var(--color-border)" }}>
        {TABS.map((tab) => (
          <button key={tab.id} onClick={() => setScope(tab.id)}
            className={cn("px-5 py-2 rounded-sm text-sm font-medium transition duration-normal", scope !== tab.id && "hover:opacity-80")}
            style={scope === tab.id ? { backgroundColor: "var(--color-success)", color: "var(--color-bg)", fontWeight: 700 } : { color: "var(--color-text-secondary)" }}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {data && !loading && (
        <>
          ...
          {data.entries.length === 0 ? (
            <div className="py-20 text-center text-sm" style={{ color: "var(--color-text-secondary)" }}>No rankings yet for this period — be the first!</div>
          ) : (
